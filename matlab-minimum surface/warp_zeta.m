function warp = warp_zeta(Iref, I, Grel, K, z)
   ...
   x2 = K * X2(1:3,:);
   x2 = x2./repmat(x2(3,:),3,1); %x2也是按行进行排列的
   I = double(I);
   Iw = interp2(I,x2(1,:),x2(2,:),'bicubic');
   ...
   set(gcf,'Position',[200,scr(4)/3,1000,420]);
   warp_img = axes('Parent',gcf,... 
    'Units','pixels',...  
    'Position',[30 10 434 383]); 
   axes(warp_img);
   imshow(uint8(Iw));
   hold on;
   pause(0.01);
   ...
   dX = Grel(1:3,1:3)*Xn./repmat(z_flat,3,1);%derivation of X2 wrt. z
   x_pos = x2(1:2,:);
   x_pos(1,:) = x_pos(1,:) + 0.5*ones(1,length(x_pos));
   Ix_pos = interp2(I,x_pos(1,:),x_pos(2,:),'bicubic');
   x_neg = x2(1:2,:);
   x_neg(1,:) = x_neg(1,:) - 0.5*ones(1,length(x_neg));
   Ix_neg = interp2(I,x_neg(1,:),x_neg(2,:),'bicubic');
   
   y_pos = x2(1:2,:);
   y_pos(2,:) = y_pos(2,:) + 0.5*ones(1,length(y_pos));
   Iy_pos = interp2(I,y_pos(1,:),y_pos(2,:),'bicubic');
   y_neg = x2(1:2,:);
   y_neg(2,:) = y_neg(2,:) - 0.5*ones(1,length(y_neg));
   Iy_neg = interp2(I,y_neg(1,:),y_neg(2,:),'bicubic');
